...
設定 → **発音辞書** から、固有名詞（ゲームタイトルやアーティスト名など）の読み方を TTS に伝える辞書を設定できます。台本本文は変更されず、読みだけを補正します。エントリは **共通（全番組）** または **この番組のみ** として登録でき、両方に同じ表記が登録されている場合は番組固有の読みが優先されます。この辞書は TTS の発声にのみ作用し、台本生成のプロンプトには一切渡らないため、実際に読まれる会話テキストは生成時のまま変わりません。
