...
Apple Music / YouTube Music / Spotify のプレイリストを素材に、AI が自動進行するラジオ番組を流す macOS アプリ。

AI が台本を作り、2 人のホストが読み上げ、音楽と重ねて再生します。
...
- macOS 14 (Sonoma) 以降
- 台本生成に使う AI CLI（いずれか 1 つをインストールしておく）
  - `claude`（Claude Code）
  - `gemini`
  - `codex`（ChatGPT Codex）
  - `copilot`
- Gemini API Key（読み上げに使用。[Google AI Studio](https://aistudio.google.com/) で無料取得可能）
...
2. **設定** → **テキスト読み上げ** タブを開く
3. **API Key** に Gemini API Key を入力する（[Google AI Studio](https://aistudio.google.com/) で無料取得）
4. **CLI** で台本生成に使う AI を選ぶ（例: `claude`）
5. 設定を閉じ、メイン画面でプレイリストを選んで **Start**

Apple Music はこれだけで動く。YouTube Music / Spotify は先にログインが必要（→ [使い方](#使い方)）。

---

## 設定ガイド

アプリを起動したら、ツールバーの **設定** ボタンを開いて各タブを設定する。

### テキスト読み上げ

最初に設定するタブ。API Key と CLI が未設定だと番組を開始できない。

| 項目 | 説明 |
|---|---|
| **API Key** | Google AI Studio で取得した Gemini API Key を入力する |
| **TTS モデル** | 読み上げに使う Gemini モデル（既定値のままで動作する） |
| **フォールバックモデル** | 主モデルが失敗したときの予備モデル |
| **男性ボイス** | 男性ホストの声（例: `Charon`） |
| **女性ボイス** | 女性ホストの声（例: `Kore`） |
| **CLI** | 台本生成に使う AI CLI を選ぶ（`claude` / `gemini` / `codex` / `copilot`） |
...
| **YouTube Music でログイン** | YouTube Music を使う場合にここからログインする |
| **Spotify でログイン** | Spotify を使う場合にここからログインする |
...
| 項目 | 説明 |
|---|---|
| **録音出力先** | 録音ファイルの保存先フォルダ。空欄なら `~/Music/AgentBooth/` に保存 |
...
画面下部の **NowPlayingBar** に現在のトラック（アートワーク付き）と番組の進行状態が表示される。

---

## 再生モード

番組情報タブの **オーバーラップモード** で選択できる。
...
システム設定 → プライバシーとセキュリティ → 自動化 を開き、**AgentBooth** の項目に **Music** の許可が入っているか確認する。

### YouTube Music / Spotify のログイン状態が「未ログイン」のまま

- 内蔵ブラウザでログインを完了してからウィンドウを閉じ、再度設定タブを開いて確認する
- ログインが途中で詰まる場合は **データを削除** を押してサイトデータを消去してから再ログインする

### Spotify でプレイリストが取得できない・再生が止まる

Spotify Web Player の画面構造が変わると動作しなくなることがある。

### 台本生成が始まらない・エラーになる

- **テキスト読み上げ**タブで選んだ CLI（`claude` など）がインストール済みか確認する
- CLI のインストール先によってはアプリから見つからない場合がある。その場合はフルパス（例: `/usr/local/bin/claude`）で CLI モデル欄に入力するか、インストール場所を確認する

### 音声（読み上げ）が生成されない

- **テキスト読み上げ**タブで Gemini **API Key** が正しく設定されているか確認する
- API Key の残量や有効期限を Google AI Studio で確認する
