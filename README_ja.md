...
オーバーラップモードがオフでも、自然終了前に曲を止める場合はフェードアウトします。番組を一時停止すると進行中のフェードもその音量で止まり、一時停止時間を含めずに再開します。
...
| **事前生成（レビュー）** | 再生前に全台本（オープニング・各トランジション・クロージング）を一括生成。自由にリサイズできる独立したレビューウィンドウで全発話テキストと TTS 演技指示を確認・編集してから再生を開始。TTS 音声は再生中にオンデマンドで生成されます |

**事前生成（レビュー）** モードの使い方:
1. **再生** を押す — 全台本が順番に生成されます（進捗はステータスバーに表示）
2. レビューウィンドウが開き、各セグメントの会話テキスト・TTS 演技指示・音声設定が表示されます。通常のウィンドウなのでリサイズ・最大化ができ、⌘W で閉じても編集内容は失われず番組も停止しません。メイン画面の「レビューを開く」リンクやウィンドウメニューから再度開けます
3. 必要に応じて会話テキストや TTS 演技指示を編集します。発話行の追加・削除、ハンドルのドラッグによる並べ替え、話者切替ができます。各行の ▶︎ ボタンではその発話だけを試聴でき（前回の行試聴から内容が変わっていなければ TTS API は消費しません）、構造変更は **元に戻す**（⌥⌘Z）で取り消せます
4. ⌘F で検索バーを開くと、全セグメントを横断してテキストを検索・置換できます
5. セグメントの「TTS へ渡る入力」を展開すると実際に TTS へ送られる内容を確認でき、**試聴** ボタンでそのセグメントを再生前に聴けます（内容が前回の試聴から変わっていなければ TTS API は消費しません）
6. **承認して再生** を押すと再生が開始されます。**キャンセル（番組を停止）** で破棄して停止します

承認済みの事前生成台本は、再生中だけ active cache としてローカルに保存されます。番組が正常終了する前に停止したりエラーになった場合、同じプレイリスト・同じ曲順で再度開始すると再利用確認が表示されます。**再利用** を選ぶと台本生成をスキップしてレビューウィンドウを再表示し、**作り直す** を選ぶと active cache をアーカイブ名へ退避して新しく生成します。番組が最後まで正常に完走したときも active cache は再利用対象から外されますが、JSON ファイル自体は履歴として cache フォルダに残ります。

### 発音辞書

設定 → **発音辞書** から、固有名詞（ゲームタイトルやアーティスト名など）の読み方を、AgentBooth が表示・保存する台本を変更せずに補正できます。エントリは **共通（全番組）** または **この番組のみ** として登録でき、両方に同じ表記が登録されている場合は番組固有の読みが優先されます。番組ごとに、原表記を保って発音ルールを追加する **発音指示として追加**（既定値）と、TTS へ送る入力だけ一致箇所を置換する **台本表記を読みへ置換** を選べます。「TTS へ渡る入力」プレビューには実際の結果が表示されます。発音辞書が台本生成プロンプトへ渡ることはありません。
