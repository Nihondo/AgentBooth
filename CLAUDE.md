...
Structural edits implemented so far: `appendLine` / `insertLine(in:after:speaker:)` / `removeLine` (refuses to delete a segment's last remaining line, to avoid sending an empty transcript to TTS) / `toggleSpeaker` / `setSpeaker(_:of:in:)` / `moveLine(_:in:offset:)` (button-driven, ±1 position) — each is one call = one undo step. Drag-and-drop reordering (`ScriptReviewSegmentEditor`, mirroring the `.onDrag`/`.onDrop(delegate:)` pattern from `TTSCredentialSetsEditor`) is a special case: `beginLineDragReorder()` pushes exactly one undo snapshot when the drag starts, and the repeated `reorderLineDuringDrag(_:in:to:)` calls fired by `DropDelegate.dropEntered` while hovering over rows do **not** push additional snapshots — otherwise a single drag gesture would flood the undo stack. `undoLastStructuralChange()` pops one snapshot (stack capped at 20) and restores `segments` wholesale, then bumps `structureRevision` once.
...
All shared value types live here: `TrackInfo`, `RadioScript`, `RadioState`, `AppSettings`, `ShowProfile` (and their sub-structs including `RadioShowSettings`, `DirectionSettings`, `BGMSettings` / `AudioAssetSource`), `TimeBand`, `OverlapMode`, `ScriptGenerationMode`, `RadioPhase`, `PrimaryControlState`, `ScriptCLIKind`, `ReviewScriptItem` (now carries a `segmentKey` used to match edited segments back to `RadioOrchestrator`'s internal `SegmentKey`, and a `pronunciationEntries` field reserved for a forthcoming TTS pronunciation dictionary feature — currently always empty), `PronunciationEntry` / `PronunciationScope` (reserved for the same forthcoming feature; not yet wired into settings, prompts, or persistence beyond the type definitions).
...
- **Applies to TTS prompts only.** `PromptBuilder` (script generation) never receives the dictionary — script text is left untouched by design.
- **`TTSInputComposer`** (`Services/TTS/TTSInputComposer.swift`) builds the actual text sent to Gemini TTS, shared by `GeminiTTSService` and the review window's input preview so "what you see is what gets sent." Block order is `Direction:` → pronunciation-dictionary block → the `Male:`/`Female:` transcript (transcript always last). The pronunciation block follows a fixed instruction template ("Follow these pronunciation rules exactly. / These rules affect pronunciation only. Do not alter the transcript." + `Pronunciation dictionary:` + `「source」 = 「reading」` lines) — phrasing it as a pronunciation-only instruction, not a find/replace instruction, avoids the CLI/model reinterpreting and rewriting the matched text in the transcript itself. The block lists **only entries that literally appear in that segment's dialogue text** (`appliesTo(entry:dialogues:)`) and is omitted entirely when none match — this keeps the no-dictionary-configured request byte-for-byte identical to before the feature existed, so `GeminiTTSServiceTests`'s pre-existing exact-match assertions on the request body still pass unmodified.
- Settings UI: `Features/Settings/PronunciationDictionaryEditor.swift` (mirrors `TTSCredentialSetsEditor`'s drag-to-reorder pattern), embedded twice in `SettingsView`'s `.pronunciation` category — once bound to `$draftSettings.globalPronunciationEntries` ("共通（全番組）") and once to `$draftSettings.directionSettings.pronunciationEntries` ("この番組のみ"). The category lives under `showProfileCategories` purely for its screen placement; the global binding it also displays there is intentional and labeled explicitly in the view, since storage scope is determined by which binding is used, not by the settings category.
- In the review window, `ReviewScriptItem.pronunciationEntries` carries the **already-resolved** effective dictionary for that segment (computed once when entering review), and on approval `RadioOrchestrator.applyEditedSegments` writes it back into `narrationSettings.directionSettings.pronunciationEntries` while zeroing `narrationSettings.globalPronunciationEntries` — mirroring how the time-band-resolved `sceneDirection` is written back — so the resolved value isn't merged a second time at playback. `restorePreGeneratedSegments(from:)` re-applies the *current* live dictionary (via `AppSettings.applyingCurrentPronunciationDictionary(from:)`) over whatever was saved in `pregenerated_session.json`, so a dictionary fix made after generation still takes effect on reuse.

**Segment preview playback** — `ScriptReviewViewModel.requestSegmentPreview(_:)` lets the reviewer hear one segment's TTS output before approving. Guardrails, in order: (1) a confirmation alert ("consumes one API call") unless the user already chose "試聴する（以後確認しない）" for this review session (`skipsPreviewConfirmation`, in-memory only — not `@AppStorage`) or the show is in test mode (`isTestMode`, which never hits the real API so confirming would be misleading); (2) a per-segment cache keyed by a content hash of scene direction + every line's speaker/text + the segment's pronunciation entries (`previewContentHash(for:)`) — an unchanged segment replays the cached WAV instead of re-synthesizing; (3) only one preview in flight at a time — `performSegmentPreview`/`playCachedPreview` always cancel `previewTask` first; (4) the "試聴" button is disabled whenever `canPreviewSegments` is false (no active TTS credential set and not in test mode). Preview always gets a **fresh** `TTSService`/`AudioPlaybackServiceProtocol` instance from `serviceFactory` (or `TestModeTTSService`/`TestModeAudioPlaybackService` under `isTestMode`) rather than reusing a shared one — `GeminiTTSService` has a same-instance `successfulCallThrottleInterval` (60s) meant to pace narration calls during a live show, and reusing that instance for previews would make the reviewer wait up to a minute between preview presses.
